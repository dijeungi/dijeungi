<h2 align="left">안녕하세요 최준호 입니다.</h2>
AI를 적극 활용해 기획, 디자인, 개발, 배포, 운영까지 제품의 전 과정을 직접 완성합니다.</br>
기능 구현뿐만 아니라 UI/UX의 완성도와 사용자 경험을 중요하게 생각하며, 작은 디테일까지 끊임없이 다듬습니다.

### Project 

- [Package-KR](https://github.com/Package-KR) : 오픈소스 Organization Package-KR 운영 (26.04.09 ~ )
  - [react-native-kakao-signin](https://github.com/Package-KR/react-native-kakao-signin) : React Native - Kakao Login Lib
  - [react-native-naver-signin](https://github.com/Package-KR/react-native-naver-signin) : React Native - Naver Login Lib

### Education

- 하이미디어 아카데미 - 디지털선도기업아카데미 | 대한상공회의소 IBM x RedHat 교육 운영 매니저 (25.05 ~ 25.09)
- 하이미디어 아카데미 - 심화 생성형 AI 활용 인재양성과정 (조별멘토) (25.03.07 ~ 25.04.30)
- 하이미디어 아카데미 - 클라우드 기반 자바 풀스택(Spring Boot, React, Flutter) 개발자 과정 (24.09 ~ 25.03)
- 학점은행제 - 컴퓨터공학 학사과정 이수 중

> *엔지니어에게 중요한 것은 문제를 해결하는 것보다, 원인을 정확히 파악하고 설명할 수 있는 과정이라고 생각합니다.*<br/>
> *조급함보다 집요함으로, 끝까지 확인하는 태도를 지향합니다.*

<br/>
<p align="left">
  <img src="https://komarev.com/ghpvc/?username=dijeungi&label=Profile%20views&color=0e75b6&style=flat" alt="dijeungi" />
</p>
